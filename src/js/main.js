...
async function initMap() {
  ...
  if (datos.length > 0 && datos != null) {
    datos.forEach((s) => {
      ...
      let i = s.id;
      var imageb64 =
        'https://i.pinimg.com/736x/3a/ab/e0/3aabe0e9a520b9ad90407a82f85adb42.jpg';
    ...
    listaUbicaciones.forEach((s) => {
      ...
      <a
                ...
                <div class="action-buttons">
                 <a onclick="cargarUbicacion(${s.id}), onVerificarEstado('edit')"> <i class="bi bi-pencil"></i>  </a>
...
function cargarUbicacion(id) {
  ...
  for (let index = 0; index < _listaUbicaciones.length; index++) {
    ...
    if (element.id == id) {
      ...
      var htmlPreview =
        '<img width="200" id="imagenBannerTemporal2" src="' +
        atob(element.imagen) +
        '" />' +
